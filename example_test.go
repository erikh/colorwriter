...
func ExampleWriter_Init() {
	...
	// touch each other).
	w.Init(os.Stdout, 5, 0, 1, ' ', colorwriter.AlignRight)
	fmt.Fprintln(w, "a\tb\tc\td\t.")
	fmt.Fprintln(w, "123\t12345\t1234567\t123456789\t.")
	fmt.Fprintln(w)
	w.Flush()
...
func Example_elastic() {
	...
	w := colorwriter.NewWriter(os.Stdout, 0, 0, 1, '.', colorwriter.AlignRight|colorwriter.Debug)
...
func Example_trailingTab() {
	...
	const padding = 3
	w := colorwriter.NewWriter(os.Stdout, 0, 0, padding, '-', colorwriter.AlignRight|colorwriter.Debug)
